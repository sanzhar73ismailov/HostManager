package instrument.sysmexCA660;
...
public class SysmexCA660Message extends instrument.Message {

    private TypeMessageSysmexCa660 typeOfMessage;
    ...
    public SysmexCA660Message(byte[] rawMessage) {
        super(rawMessage);

    }

    public SysmexCA660Message(List<BlockSysmexCA660> listBlocks) throws InstrumentException {
        super(null);
        for (int i = 0; i < listBlocks.size(); i++) {
            BlockSysmexCA660 block = listBlocks.get(i);
            this.params = block.getParameters();
            if (i == 0) {
                switch (params.textDistinctionCode1) {
                    case "D":
                        typeOfMessage = TypeMessageSysmexCa660.RESULT;
                        break;
                    case "R":
                        typeOfMessage = TypeMessageSysmexCa660.QUERY;
                        break;
                    case "S":
                        typeOfMessage = TypeMessageSysmexCa660.ORDER;
                        break;
                    default:
                        throw new instrument.InstrumentException("Unknown type of Message: " + params.textDistinctionCode1);
                ...
                try {
                    ...
                    Logger.getLogger(SysmexCA660Message.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public String createOrderEmptyFromQueryMessage() {
        ...
        String str = STX_STRING
                ...
                + util.Util.getStringFillBySpace(11)
                + parameterCode + util.Util.getStringFillBySpace(6)
                + ETX_STRING;
        return str;
    }
    ...
    private static String getPatientName(WorkOrder wo) {
        String patientName = util.Util.getStringFillBySpace(11);
        ...
        if (pName != null && !pName.trim().isEmpty()) {
            ...
            patientName = util.Util.getStringFillBySpace(numberOfChars - pName.length()) + pName;
        }
        return patientName;
    }
    ...
    private void setDataFromParams(Parameters parameters) {
        for (String atrArrayData2 : parameters.data) {
            ...
            if (this.typeOfMessage == TypeMessageSysmexCa660.RESULT) {
    ...
    public TypeMessageSysmexCa660 getTypeOfMessage() {
